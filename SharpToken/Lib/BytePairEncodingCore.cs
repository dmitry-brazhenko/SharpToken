...
public class BytePairEncodingCore
{
    ...
    {
        return Decoder.TryGetValue(token, out tokenBytes) || SpecialTokensDecoder.TryGetValue(token, out tokenBytes);
    }
        ...
        IReadOnlyCollection<byte> piece, IReadOnlyDictionary<byte[], int> ranks, Func<Range, T> f)
    {
        ...
        int? GetRank(IReadOnlyList<(int Start, int Rank)> partitionsList, int startIndex, int skip)
        {
            ...
            return ranks.TryGetValue(key, out var rank) ? rank : null;
        }
